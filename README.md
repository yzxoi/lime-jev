...
这次更新减少逐键词组搜索，在文字上屏后提前计算下一段的上下文，并更新 Lua 的候选等待上限。方案仍选 **Lime · 本地语境**，候选策略仍推荐 `./lime-jev use lime`。输入框中的候选会在下一次输入时使用新服务；若仍在原来的组合状态，先按 Esc。

逐键搜索只扩展最可能的三个起始分支；通常每键最多新增一次模型推理，首次完整词组最多两次，短词与长句共享缓存。搜索启动预算为 50 ms，**不是每键总耗时保证**，已开始的原生推理不能中断。一次回复最多返回 64 个模型候选，其后仍有 Rime 词典候选。异常情况下，Lua 等待候选最多 250 ms 后使用词典回退。具体对照结果和限制见 [延迟验证](docs/LATENCY.md)。
...
- 探索多 token 词组，使“油 + 箱”“又 + 想”等词组进入候选，使用上下文条件概率排序；逐键限制新增推理次数，短词与长句复用同一分支缓存。
